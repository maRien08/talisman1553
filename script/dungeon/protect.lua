    ...
    elseif self.CurrState == 2 then
        ...
        if aliveMassesCount == 0 then
            if (self.CurrMassLevel >= TD_MaxSpawnLevel) then
                ...
                local LAN_SPAWN_NEXT = LAN("msg_script_protect_spawn"); -- "下一波进攻将在%d秒后出现";
                local strNextSpawn = string.format(LAN_SPAWN_NEXT, 1);
                --DungeonFactionReceiveMsg(dungeon, DUNGEON_FACTION_ALL, strNextSpawn, CHANNEL_SYS);
                DungeonFactionReceiveBulletin(dungeon, DUNGEON_FACTION1, strNextSpawn);
            end
        end
    end
...
function CDungeon_TD:OnDungeonBegin(dungeon)
    ...
    self.Protector = CreateNpcInDungeon(self.ObjectId, 4692, DUNGEON_FACTION1, 627, 41, 0, self.ActorLevel , 10)

    --商店
    --CreateNpcInDungeon(self.ObjectId, 4700, DUNGEON_FACTION_OBSERVER, 615, 41, 0, 0, 1)
...
function CDungeon_TD:OnDungeonEnd(dungeon,winner)
    ...
    self.TD_Nimbus = GetMonsterNimbus (self.ActorLevel) * self.TD_KillNum
    ...
    if (DungeonIsTrainMode(dungeon) == 1) then
        if (winner == DUNGEON_FACTION1) then -- 赢了	
            ...
            for i=0,total-1 do
                ...
                --AddExp (actor,self.TD_Exp*0.1);
                AddNimbus (actor,self.TD_Nimbus*0.2);
                --AddMoney (actor,self.TD_Money*0.1);
            end
            
            contentwt = string.gsub(contentwt, "$DEFENCE_COUNT", passLevel);
            contentwt = string.gsub(contentwt, "$TD_Score", self.TD_Score);                        
            contentwt = string.gsub(contentwt, "$TD_Exp", self.TD_Exp*0.1);
            contentwt = string.gsub(contentwt, "$TD_Nimbus", math.floor(self.TD_Nimbus*0.2));
            contentwt = string.gsub(contentwt, "$TD_Money", self.TD_Money*0.1); 
            contentwt = string.gsub(contentwt, "$TD_God", self.TD_God);
        ...
        elseif (winner == DUNGEON_FACTION2) then -- 输了
            if (self.CurrMassLevel >= 1) then
                ...
                contentlt = string.gsub(contentlt, "$TD_Score", self.TD_Score); 
                contentlt = string.gsub(contentlt, "$TD_Exp", self.TD_Exp);
                contentlt = string.gsub(contentlt, "$TD_Nimbus", math.floor(self.TD_Nimbus*0.2));
                contentlt = string.gsub(contentlt, "$TD_Money", self.TD_Money);
                contentlt = string.gsub(contentlt, "$TD_God", self.TD_God);
    ...
    if (DungeonIsTrainMode(dungeon) ~= 1) then
        ...
        self.TD_Exp = GetMonsterExp (self.ActorLevel) * self.TD_KillNum
        self.TD_Money = GetMonsterGold (self.ActorLevel) * self.TD_KillNum
        ...
        if (winner == DUNGEON_FACTION1) then -- 赢了	
            ...
            for i=0,total-1 do
                ...
                AddExp (actor,self.TD_Exp*0.5);
                AddNimbus (actor,self.TD_Nimbus*0.5);
            ...
            contentws = string.gsub(contentws, "$TD_Nimbus", math.floor(self.TD_Nimbus*0.5));
        ...
        elseif (winner == DUNGEON_FACTION2) then -- 输了
            ...
            if (self.CurrMassLevel >= 1) then
                ...
                contentls = string.gsub(contentls, "$TD_Nimbus", math.floor(self.TD_Nimbus*0.5));
...
function CDungeon_TD:SpawnMass()
    ...
    --for i=1,massCount do
        --CreateNpcInDungeon(self.ObjectId, TD_SpawnNpc[self.CurrMassLevel], DUNGEON_FACTION2, x + delta_x * i, y + delta_y * i, 0, self.ActorLevel , 0.6+self.CurrMassLevel * 0.2)
    --end

    CreateNpcInDungeon(self.ObjectId, TD_SpawnNpc[self.CurrMassLevel], DUNGEON_FACTION2, x, y, 0, self.ActorLevel , 0.6+self.CurrMassLevel * 0.2)
...
function CDungeon_TD:SpawnBoss()
    ...
    CreateNpcInDungeon(self.ObjectId, TD_SpawnBoss[temp + 1], DUNGEON_FACTION2, x, y, 0, self.ActorLevel , 0.8+self.CurrMassLevel * 0.3)
...
function CDungeon_TD:SpawnLeftAndRight()
    ...
    if (math.mod(self.CurrMassLevel, 5) == 3) then
        ...
        for i=1,spawn_left do
            CreateNpcInDungeon(self.ObjectId, TD_SpawnNpc_Left[self.CurrMassLevel], DUNGEON_FACTION2, x_left, y_left, 0, self.ActorLevel , 0.6+self.CurrMassLevel * 0.2)
        end

        for i=1,spawn_right do
            CreateNpcInDungeon(self.ObjectId, TD_SpawnNpc_Right[self.CurrMassLevel], DUNGEON_FACTION2, x_right, y_right, 0, self.ActorLevel , 0.6+self.CurrMassLevel * 0.2)
        end
